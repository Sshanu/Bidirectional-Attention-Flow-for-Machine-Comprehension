...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        self.pos_x = tf.placeholder('int32', [N, None, None], name='pos_x')
        self.pos_q = tf.placeholder('int32', [N, None], name='pos_q')
    ...
    def _build_forward(self):
        ...
        VP = config.pos_vocab_size 
        dp = config.pos_emb_size
        ...
        with tf.variable_scope("emb"):
            ...
            with tf.name_scope("pos"):
                Px = tf.nn.embedding_lookup(pos_emb_mat, self.pos_x)  # [N, M, JX, d]
                Pq = tf.nn.embedding_lookup(pos_emb_mat, self.pos_q)  # [N, JQ, d]
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        pos_x = pos2int['OTHER']*np.ones([N, M, JX], dtype='int32')
        pos_q = pos2int['OTHER']*np.ones([N, JQ], dtype='int32')
        ...
        feed_dict[self.pos_x] = pos_x
        feed_dict[self.pos_q] = pos_q
            ...
            for j, pp in enumerate(pos_tags):
                for k, ppp in enumerate(pp):
                    if k == config.max_sent_size:
                        break
                    # print(idx, i,j,k, pos_tags[j][k])
                    pos_x[i, j, k] = pos2int[pos_tags[j][k]]

        for i in range(len(batch.data['q'])):
            idx = batch.data['idxs'][i]
            pos_tag = batch.shared['pos_q'][idx]
            for j, pp in enumerate(pos_tag):
                pos_q[i,j] = pos2int[pp]
